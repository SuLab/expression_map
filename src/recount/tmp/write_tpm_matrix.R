...
## data.mat <- cbind(tcga.tpm,gtex.tpm)
...
##     data.mat <- cbind(data.mat,proj.tpm)

## }

## saveRDS(data.mat,'/gpfs/group/su/lhgioia/map/data/recount/genes_by_samples_tpm.RDS')

data.mat <- readRDS('/gpfs/group/su/lhgioia/map/data/recount/genes_by_samples_tpm.RDS')

## write.table(rownames(data.mat),'/gpfs/group/su/lhgioia/map/data/recount/sample_list.txt')

write.table(data.mat[1,,drop=F],'/gpfs/group/su/lhgioia/map/data/recount/genes_by_samples_tpm.csv',sep=',',row.names=T)

for(i in 2:nrow(data.mat)){

    write.table(data.mat[i,,drop=F],'/gpfs/group/su/lhgioia/map/data/recount/genes_by_samples_tpm.csv',sep=',',row.names=T,append=T,col.names=F)

}

## dim(data.mat)

## write.table(data.mat,'/gpfs/group/su/lhgioia/map/data/recount/genes_by_samples_tpm.csv',sep=',')
