...
recount.data <- readRDS('results/recount/pca/recount_250_dim_noScaled_noProj_over50_noSingle_entrez.RDS')
...
for(perp in perplexity.vec){
    for(dim in dim.vec){
        ...
        saveRDS(results,'results/recount/tsne/recount_tsne_pca_list_noNAs_over50_noSingle_entrez.RDS')
