...
kmeans.results <- readRDS('results/recount/clustering/kmeans_1to100_recount_250_dim_noScaled_noProj_over50_noSingle.RDS')
cluster.ass <- kmeans.results[[cluster.num]]
...
for(i in 1:cluster.num){
    for(j in (i+1):cluster.num){

        if(i == j | j > cluster.num){
            next
        }

        first.clust.samps <- intersect(rownames(tpm.dat),names(which(cluster.ass==i)))
        second.clust.samps <- intersect(rownames(tpm.dat),names(which(cluster.ass==j)))
        ...
        out.dat <- apply(first.clust.dat[,genes.use,drop=F],2,function(x) log2(mean(x+1)))
        in.dat <- apply(second.clust.dat[,genes.use,drop=F],2,function(x) log2(mean(x+1)))
        ...
        pval.res[[sprintf('%d %d',i,j)]] <- data.frame(pvalue = sapply(gene.tests, function(x) x$p.value),
                                                       statistic = sapply(gene.tests,function(x) x$statistic),
                                                       fold.change = total.diff)
            ## c(gene.tests$p.value,gene.tests$statistic)

        print(sprintf('Completed cluster %d %d',i,j))

        saveRDS(pval.res,'results/recount/markers/kmeans30_ttest_markers_pairwise.RDS')
    }
}
