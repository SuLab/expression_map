pickSoftThreshold <- function (data, dataIsExpr = TRUE, RsquaredCut = 0.85, powerVector = c(seq(1, 10, by = 1), seq(12, 20, by = 2)), removeFirst = FALSE, nBreaks = 10, 
                               blockSize = NULL, corFnc = cor, corOptions = list(use = "p"), 
                               networkType = "unsigned", moreNetworkConcepts = FALSE, gcInterval = NULL, 
                               verbose = 0, indent = 0) 
{
    ...
    if (verbose == 1) 
        printFlush("")
    saveRDS(datk,'data/allen/datk_tmp.RDS')
