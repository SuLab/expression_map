...
for(i in unique(cluster.vec)){
    ...
    pct.express.in <- round(apply(tpm.dat[group.vec=='Inside',,drop=F],2,function(x) sum(x>0) / length(x)),digits=3)
    ...
    p.val <- tryCatch(
    {
        ...
        apply(tpm.dat[,genes.use,drop=F],2,function(x) wilcox.exact(x ~ as.factor(group.vec))$p.value)
    },
        error = function(cond) {
            print(i)
