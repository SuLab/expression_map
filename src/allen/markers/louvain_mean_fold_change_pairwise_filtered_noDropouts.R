...
for(i in 1:length(unique(cluster.vec))){
    for(j in (i+1):length(unique(cluster.vec))){
        ...
        first.clust.dat <- apply(first.clust.dat,2,function(x) log(mean(x+1)))
        second.clust.dat <- apply(second.clust.dat,2,function(x) log(mean(x+1)))
        ...
        saveRDS(fc.res,'results/allen/markers/louvain_mean_fold_change_pairwise_filtered_noDropouts.RDS')
    }
}
