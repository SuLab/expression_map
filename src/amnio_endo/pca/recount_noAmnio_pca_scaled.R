...
recount.meta$run_id <- sub('.bw','',recount.meta$run_id)
...
saveRDS(gene.pca,'/gpfs/group/su/lhgioia/map/results/amnio_endo/pca/recount_proj_noAmnio_big_irlba_scaled.RDS')

corr.gene.dat <- as.matrix(big.data %*% gene.pca$v)
rownames(corr.gene.dat) <- rownames(big.data)

saveRDS(corr.gene.dat,'/gpfs/group/su/lhgioia/map/results/amnio_endo/pca/recount_noAmnio_50_dim_irlba_scaled.RDS')
